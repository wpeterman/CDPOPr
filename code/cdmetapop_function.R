...
cdpop <- function(CDPOP.py,
  ...
  read.grid <- function(grid){
    ...
    cd_df <- as.data.frame(cdpop_out[,-1])
    
    ## If simulating msats with many loci, will need to update `ncode`
    # ncode <- ifelse(alleles > 9, 2, 1)
    ncode <- 1
  ...
  names(grid_list) <- paste0('gen_', sort(gens))
